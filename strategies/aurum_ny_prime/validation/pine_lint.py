"""Static checks for the AURUM-NY PRIME Pine v6 source.
...
* `lookahead_on`, or a `request.security()` with no explicit lookahead, either
  of which silently introduces future data;
* stateful helpers evaluated inside `request.security()`;
* v5-era removals (`iff`, `security(`, `study(`, `crossover(` bare forms);
* tabs, or indentation that is not a multiple of four;
* a mandatory condition from the specification missing from the entry gate;
* a diagnostic string from S59 that no longer appears in the source.

Run directly: ``python -m validation.pine_lint path/to/AURUM_NY_PRIME.pine``
"""
...
#: Every condition S55 declares mandatory must appear in the entry gate chain.
MANDATORY_GATES = (
    "goldBullGate",
    "sessionGate",
    "orbGate",
...
#: S59 requires the engine to be able to say why it did not trade.
...
def lint(source: str) -> list[Finding]:
    ...
    gate_line = next((line for line in lines if line.startswith("armed = ")), "")
    gate_blob = " ".join(
        line for line in lines
        if line.startswith(("armed = ", "technicalValid", "intermarketOK", "executionOK", "riskValid"))
    )
    if not gate_line:
        findings.append(Finding(0, "spec", "no `armed = ` entry gate found"))
    for gate in MANDATORY_GATES:
        if gate not in gate_blob:
            findings.append(Finding(0, "spec", f"mandatory condition `{gate}` (S55) missing from the entry gate"))
    for diagnostic in REQUIRED_DIAGNOSTICS:
        if diagnostic not in source:
            findings.append(Finding(0, "spec", f"diagnostic string missing (S59): {diagnostic}"))

    # --- long-only ------------------------------------------------------
    if "strategy.short" in source:
        findings.append(Finding(0, "spec", "strategy.short appears; the specification is long-only (S3)"))
